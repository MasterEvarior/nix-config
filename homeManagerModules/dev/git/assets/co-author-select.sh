...
OPTIONS_LABELS=("Claude" "Copilot" "Cursor" "Devin" "OpenCode" "Gemini")
OPTIONS_VALUES=("Claude <noreply@anthropic.com>" "Copilot <copilot@github.com>" "Cursor <cursor@cursor.com>" "Devin AI <devin-ai[bot]@users.noreply.github.com>" "OpenCode <noreply@opencode.ai>" "Gemini <gemini@google.com>")
...
if [ ${#CHECKED[@]} -eq 0 ]; then
    echo "No co-authors selected. Leaving commit as is."
    exit 0
fi
...
trailers=""
for author in "${CHECKED[@]}"; do
    trailers="${trailers}co-authored-by: ${author}"$'\n'
done

new_msg=$(printf "%s\n\n%s" "$existing_msg" "$trailers")
